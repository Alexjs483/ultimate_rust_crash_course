...
fn main() {
    ...
    // Here the input closure paramater list must be empty, and the body must be an expression
    // NOTE: since expensive_sum takes the input vector by value,
    // the closure will automatically become a move closure --> I write is explicitly
